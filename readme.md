...
현재 과제 마감 전날 저녘 11시 업데이트입니다.
현재까지 ui겹침 문제를 해결하고 자잘한 여러가지 경로나 코드내에서의 중복을 제거하는데 시간을 많이 썻습니다.
리플레이를 제외한 게임은 2월 10일에서 14일 안에 만들고 리플레이는 ui나 자잘한 버그가 있어서 조금 오래걸렸습니다.
현재 ui는 원래 안겹치는 선에서 오른쪽 위에 두려고 했으나 여유공간을 고려하여 아래에 배치하고 셀은 전체 창의 크기에 비례하여 칸을 채우도록
하였습니다.

전체적으로 감상평을 하자면 이번 과제로 꽤나 많을 걸 배웠습니다.
프로젝트에 마감기한이나 자주 커밋을 하는 것 그리고 살면서 이렇게 커밋을 많이 하다보니 여러가지
깃허브 오류를 봤습니다.
파일을 통쨰로 두번정도 날렸었다 복구했습니다.
또한 이번에 코드 자체는 간단하지만 전체적으로 기획하고 직접적으로 계속 하다보니 여러 부분에서 생각하지 못한 부분에서 오류가 나보기도 하니까 재밌고 어렵기도 하였습니다.
ui문제는 검색이나 여러가지를 동원하다 최근에 추천받은 opencode와 oh-my-opencode를 써봤는데 제가 한 것보다 더 깔끔했습니다.
맥 환경에서 돌려보기 위해 파일을 욺기고 했을때 발생한 오류도 해결할 수 있게 도와주었습니다.

https://github.com/dudgh1101/secu_result_mac
맥에서 최근에 작업한 코드들 입니다.
opencode와 플러그인을 써보기 위해 만들었고 현재는 자잘한 오류들을 .md파일에 정리하고 ui관련한 문제점도 여기서 보실 수 있습니다.
처음에는 ui문제를 해결하고 보려고 보고서로 봤는데 너무 깔끔해서 다른 오류들이 있는지 더 찾고 .md 파일로 만들어달라 요청한 결과물도 보실 수 있습니다.

아마 현직 개발자 분들께서는 제 코드가 만족스럽지 않으시겠지만
제가 할 수 있는 최선을 다한거 같습니다.
앞으로도 씨큐레이어에서 배워가며 더욱 노력하도록 하겠습니다.
2026-02-11