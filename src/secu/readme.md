...
* 이동 도착 밑 아이템 검사 *
  ...
  handleKeyPress에서 미리 각 키에대한 이벤트 지정
...
* 과제 2. 맵 생성 규칙 강화
  ...
  * 기존에 시야를 밣히는 로직과 똑같이 복붙하고 플레이어 클래스에 hastrap과 trapcountdown을 추가하고 현재는 타이머들을 초기화하는 곳에 로직만 추가하고 다음에 마저 구현할 계획입니다.
  * 마저 구현할 떄는 테스트 메즈파일을 기반으로 덫을 테스트할 계획입니다.
  * (함수,키리스너,타이머,테스트,제네레이터(덫 배치완료, 생성규칙 강화부터)<-여기부터)
  * 
